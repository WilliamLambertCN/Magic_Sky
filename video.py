...
sky_num=str(5)
sky_path='d:/projects/DIP/Final_Project/Unet/sky/'+sky_num+'.jpg'
sky=cv2.imread(sky_path)
sky = cv2.cvtColor(sky, cv2.COLOR_BGR2RGB)


video = cv2.VideoCapture("d:/projects/DIP/Final_Project/Unet/Demo/video_test.mp4")
fps = video.get(cv2.CAP_PROP_FPS)
frameCount = video.get(cv2.CAP_PROP_FRAME_COUNT)
size = (int(video.get(cv2.CAP_PROP_FRAME_WIDTH)), int(video.get(cv2.CAP_PROP_FRAME_HEIGHT)))

videoWriter = cv2.VideoWriter('d:/projects/DIP/Final_Project/Unet/Demo/trans.mp4', cv2.VideoWriter_fourcc(*'MP4V'), fps, size)  
